...
public class CalculatorSimpleTest {
  ...
  /** Adding zero to a number should always
    ...
    */
  @ParameterizedTest
  @ValueSource(	ints = { -99, -1, 0, 1, 2, 101, 337, 200  })
  ...
  @ParameterizedTest
  @ValueSource(	ints = { -99, -1, 0, 1, 2, 101, 337, 200  })
  void testSubtract_ParameterizedTest(int num){
      Calculator c = new Calculator(num, 0);
      int result = c.subtract();
    assertEquals(result, num, "result should be same as num");
  }
  ...
  @ParameterizedTest
  @MethodSource("additionTestCasesProvider_subtract")
  void testSubtract_MethodSource(int num1, int num2, int expectedResult){
    Calculator c = new  Calculator(num1, num2);
    int result = c.subtract();
    assertEquals(result,expectedResult,"result should be same as as expected result");
  }

  static Stream<Arguments> additionTestCasesProvider_subtract(){
      return Stream.of(
          arguments(3,1,2),
          arguments(10,3,7),
          arguments(3,6,-3)
